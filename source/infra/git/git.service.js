...
export class GitService {
    ...
    async clone(repository, branch, targetPath) {
    ...
    async check(projectPath, branch) {
        const logs = [];
        const repository = await this.getOriginUrl(projectPath);
        const authenticatedRepository = this.withToken(repository);
        const fetch = await this.shell.run("git", ["fetch", authenticatedRepository, branch], projectPath);
        logs.push(this.format(fetch.command, fetch.stdout, fetch.stderr));
        if (fetch.exitCode !== 0) {
            throw new Error(this.errorMessage("git fetch failed", fetch.command, fetch.exitCode, fetch.stdout, fetch.stderr));
        }
    ...
    async pull(projectPath, branch) {
        const repository = await this.getOriginUrl(projectPath);
        const authenticatedRepository = this.withToken(repository);
        return this.shell.run("git", ["pull", authenticatedRepository, branch], projectPath);
    }
    async getOriginUrl(projectPath) {
    ...
    withToken(repository) {
        ...
        return repository.replace("https://github.com/", `https://x-access-token:${env.GITHUB_TOKEN}@github.com/`);
    }
    format(command, stdout, stderr) {
        return [`$ ${this.maskToken(command)}`, stdout, stderr].filter(Boolean).join("\n");
    }
    errorMessage(label, command, exitCode, stdout, stderr) {
        return [
            label,
